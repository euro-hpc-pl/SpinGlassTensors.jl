...
#=
function qr_fact(M::AbstractMatrix{T}, Dcut::Int=typemax(Int), tol::T=eps(); toGPU=true, kwargs...) where T <: Real
    if typeof(M) <: CuArray M = Array(M) end
    q, r = qr_fix(qr(M; kwargs...))  # QR is always done on CPU
    if toGPU q, r = CuArray.((q, r)) end
    Dcut >= size(q, 2) && return q, r
    U, Σ, V = svd(r, Dcut, tol)
    if toGPU
        return CuArray.((q * U, Σ .* V'))
    else
        return q * U, Σ .* V'
    end
end
...
function rq_fact(M::AbstractMatrix{T}, Dcut::Int=typemax(Int), tol::T=eps(); toGPU=true, kwargs...) where T <: Real
