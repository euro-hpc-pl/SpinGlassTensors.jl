...
function attach_central_left(L::ArrayOrCuArray{T, 3}, M::SparseCentralTensor{T}) where T <: Real
...
function attach_central_right(R::ArrayOrCuArray{T, 3}, M::SparseCentralTensor{T}) where T <: Real
...
function update_reduced_env_right(RR::Array{T, 2}, M::SparseCentralTensor{T}) where T <: Real
    RR = reshape(RR, size(RR, 1), size(RR, 2), 1)
    RR = attach_central_right(RR, M)
    dropdims(RR, dims=3)
end

function _project_on_border(K::Array{T, 1}, M::SparseCentralTensor{T}) where T <: Real
