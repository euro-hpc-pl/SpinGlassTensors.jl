...
"""
 ...
 L = LE -- M --
      |    |
        -- B --
"""
function update_env_left(LE::S, A::S, M::T, B::S) where {S <: Array{R, 3}, T <: Array{R, 4}} where R <: Real
...
"""
 ...
 R =  -- M -- RE
      ...
      -- B --
"""
function update_env_right(RE::S, A::S, M::T, B::S) where {T <: Array{F, 4}, S <: Array{F, 3}} where F <: Real
...
"""
  ...
  LE -- M -- RE
   |    |    |
     -- B --
"""
function project_ket_on_bra(LE::S, B::S, M::T, RE::S) where {T <: Array{R, 4}, S <: Array{R, 3}} where R <: Real
...
function corner_matrix(C::S, M::T, B::S) where {S <: Array{R, 3}, T <: Array{R, 4}} where R <: Real
