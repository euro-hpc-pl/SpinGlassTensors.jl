export compress!, _left_nbrs_site, _right_nbrs_site
...
function update_env_right(
    RE::S, A::S, M::T, B::S
) where {T <: AbstractArray{Float64, 4}, S} # where {T <: AbstractArray{Float64, 4}, S <: AbstractArray{Float64, 3}}
...
) where {T <: AbstractArray{Float64, 4}, S} # {T <: AbstractArray{Float64, 4}, S <: AbstractArray{Float64, 3}}
    @tensor R[nt, nc, nb] := RE[ot, oc, ob] * A[nt, α, ot] *
...
function update_env_right(
    RE::S, A::S, M::T, B::S
) where  {T <: SparseVirtualTensor, S} #  {T <: SparseVirtualTensor, S <: AbstractArray{Float64,3}}
    h = M.con
    p_lb, p_l, p_lt, p_rb, p_r, p_rt = M.projs
    @cast A4[x, k, l, y] := A[x, (k, l), y] (k ∈ 1:maximum(p_rt))
    @cast B4[x, k, l, y] := B[x, (k, l), y] (k ∈ 1:maximum(p_lb))
